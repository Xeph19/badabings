...
class RevenueOverrideController extends Controller
{
    ...
    public function upsert(Request $request)
    {
        ...
        if ($override->period_type === 'daily') {
            $this->generateOrdersForSingleDay($override->period_date, $override->override_amount, $menuItems, $discounts);
        } else {
            // Monthly override
            $monthStart = Carbon::parse($override->period_date)->startOfMonth();
            $monthEnd = Carbon::parse($override->period_date)->endOfMonth();
            ...
            $tempSum = 0;
            for ($day = 1; $day <= $numDays; $day++) {
                $dailyAmt = round(($dailyWeights[$day] / $totalWeight) * $override->override_amount, 2);
                $dailyAmounts[$day] = $dailyAmt;
                $tempSum += $dailyAmt;
            }
            // Adjust discrepancy
            $diff = round($override->override_amount - $tempSum, 2);
